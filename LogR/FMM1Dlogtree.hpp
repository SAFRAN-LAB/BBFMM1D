...
class FMM1DTree{
    ...
    std::vector<double> boxLogRadius;
				...
				tree[j][k].locals	=	Eigen::VectorXd::Zero(rank);
					...
					for (int l=0; l<2; ++l) {
						...
						if (nInner>-1) {
							...
							tree[j][k].locals+=boxLogRadius[j]*tree[j][nInner].multipoles.sum()*Eigen::VectorXd::Ones(rank);
						}
					}
					...
					for (int l=0; l<2; ++l) {
						...
						if (nOuter>-1) {
							...
							tree[j][k].locals+=boxLogRadius[j]*tree[j][nOuter].multipoles.sum()*Eigen::VectorXd::Ones(rank);
						}
					}
			}
			//std::cout << "==========" << std::endl;
